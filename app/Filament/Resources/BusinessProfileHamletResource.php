...
{
    protected static ?string $model = BusinessProfileHamlet::class;

    protected static ?string $navigationIcon = 'heroicon-o-rectangle-stack';

    protected static ?string $navigationLabel = 'Profil UMKM Dusun';

    protected static ?int $navigationSort = 2;

    protected static ?string $pluralModelLabel = 'Profil UMKM Dusun';

    protected static ?string $modelLabel = 'Profil UMKM Dusun';

    protected static ?string $slug = 'profil-umkm-dusun';
    ...
    public static function getNavigationBadge(): ?string
    {
        ...
        if ($user && $user->hasRole('kepala_dusun')) {
            ...
            $count = static::getModel()::where('approved', 1)
    ...
    public static function table(Table $table): Table
    {
        $queryModifier = function (Builder $query) {
            ...
            // Check if the user has a related chief_hamlet record
            if ($user->chief_hamlet) {
                // If the user is a chief_hamlet, filter records based on hamlet_id
                $query->where('hamlet_id', $user->chief_hamlet->hamlet_id);
            } else {
                // If the user is not a chief_hamlet or has no related record, return no records
                $query->where('user_id', 0);  // Assuming no user has an ID of 0
            }
